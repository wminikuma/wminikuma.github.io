...
> **(생각)** 슬라이스의 고유의 기능이다. 슬라이스는 배열과 다르게 동적으로 크기를 늘릴 수 있다고 하였다. ```append()```를 사용하여 선언된 크기보다 더 큰 요소를 추가할 수 있다. 여기서 중요한 점은 새로운 슬라이스를 만든다는 점이다.{: .prompt-info }      
...
> **(생각)** 복사는 새로운 공간에 값을 복사하는 거다. 여기서 값은 포인터, 구조체, 배열, 필드등과 같은 거라 보면 된다.{: .prompt-info }
...
> **(중요)** 새로운 공간을 가지는 배열을 만들게 되면 실제 배열 공간도 새로운 크기에 맞게 새롭게 만들어진다.{: .prompt-info }         
